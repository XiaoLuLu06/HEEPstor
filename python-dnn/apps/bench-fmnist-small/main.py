...
def train_model(model, train_loader, test_loader, criterion, device,
                num_epochs=10, lr=0.01, checkpoint_path='mnist_model.pth'):
...
def get_test_predictions(model, test_loader, device, n_samples: int):
    ...
    for i, ax in enumerate(axes):
        ...
        ax.set_title(
            f'True: {classes[labels[i].item()]}\nPred: {classes[predictions[i].item()]} (prob {probs[i, predictions[i]].item() * 100:.2f}%)')
        ax.axis('off')
    plt.tight_layout()
    plt.show()
...
def main(retrain, use_gpu_if_available):
    ...
    cg = hp.code_generator.CodeGenerator('bench-fmnist-small', hp_nn)
