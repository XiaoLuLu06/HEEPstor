...
public:
    ...
    static constexpr size_t INPUT_HEIGHT = 15;
    static constexpr size_t INPUT_WIDTH = 15;
    ...
    static void infer(SystolicArray& systolic_array, const Matrix<float>& inputs, Matrix<float>& outputs,
        ...
        // conv2: Conv2d
        const PackedInt8Matrix conv2_kernel_weights = PackedInt8Matrix::from_const_pointer(ModelParameters::conv2_kernel_weight_data, 256, 6);
        const Matrix<float> conv2_bias = Matrix<float>::from_const_pointer(ModelParameters::conv2_bias_data, 1, 6);
        ...
        float* pong_buffer = StaticArenaAllocator::allocate_array<float>(384);

        float* im2row_buffer = StaticArenaAllocator::allocate_array<float>(16384);

        Matrix<float> intermediate_buf_1(ping_buffer, 11*11, 16);
        Matrix<float> intermediate_buf_2(pong_buffer, 8*8, 6);
        Matrix<float> intermediate_buf_3(ping_buffer, 1, 384);
        ...
        // 1. conv1: Conv2d
        Conv2d::forward(systolic_array, inputs, conv1_kernel_weights, ModelParameters::conv1_kernel_weight_scale, conv1_bias, intermediate_buf_1, 5, 1, 15, 15, im2row_buffer);
