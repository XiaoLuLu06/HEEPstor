...
public:
    ...
    static constexpr size_t INPUT_HEIGHT = 28;
    static constexpr size_t INPUT_WIDTH = 28;
    ...
    static void infer(SystolicArray& systolic_array, const Matrix<float>& inputs, Matrix<float>& outputs,
        ...
        // conv1: Conv2d
        const PackedInt8Matrix conv1_kernel_weights = PackedInt8Matrix::from_const_pointer(ModelParameters::conv1_kernel_weight_data, 25, 16);
        ...
        // fc1: Linear
        const PackedInt8Matrix fc1_weights = PackedInt8Matrix::from_const_pointer(ModelParameters::fc1_weight_data, 384, 10);
        ...
        float* ping_buffer = StaticArenaAllocator::allocate_array<float>(9216);
        float* pong_buffer = StaticArenaAllocator::allocate_array<float>(11616);

        float* im2row_buffer = StaticArenaAllocator::allocate_array<float>(69696);

        Matrix<float> intermediate_buf_1(ping_buffer, 24*24, 16);
        Matrix<float> intermediate_buf_2(pong_buffer, 22*22, 24);
        Matrix<float> intermediate_buf_3(ping_buffer, 11*11, 24);
        Matrix<float> intermediate_buf_4(pong_buffer, 8*8, 6);
        Matrix<float> intermediate_buf_5(ping_buffer, 1, 384);
        ...
        // 1. conv1: Conv2d
        Conv2d::forward(systolic_array, inputs, conv1_kernel_weights, ModelParameters::conv1_kernel_weight_scale, conv1_bias, intermediate_buf_1, 5, 1, 28, 28, im2row_buffer);
        ...
        // 4. conv2: Conv2d
        Conv2d::forward(systolic_array, intermediate_buf_1, conv2_kernel_weights, ModelParameters::conv2_kernel_weight_scale, conv2_bias, intermediate_buf_2, 3, 16, 24, 24, im2row_buffer);
        performance_timer.checkpoint();
        ...
        // 7. pool2: MaxPool2d
        MaxPool2d::forward(intermediate_buf_2, intermediate_buf_3, 2, 22, 22);
        performance_timer.checkpoint();

        // 8. conv3: Conv2d
        Conv2d::forward(systolic_array, intermediate_buf_3, conv3_kernel_weights, ModelParameters::conv3_kernel_weight_scale, conv3_bias, intermediate_buf_4, 4, 24, 11, 11, im2row_buffer);
        performance_timer.checkpoint();
