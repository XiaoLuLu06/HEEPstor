...
private:
    ...
    __attribute__((always_inline)) uint32_t read_32_bits(ptrdiff_t offset) { return mmio_region_read32(base_addr, offset); }

    __attribute__((always_inline)) void write_32_bits(ptrdiff_t offset, uint32_t val) { mmio_region_write32(base_addr, offset, val); }
};